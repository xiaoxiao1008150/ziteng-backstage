...
export default {
  ...
  created() {
    if (this.type != 'mobile') {
      this.provinces = this.getDistricts()//获取省数据 开始默认是[]
      this.cities = this.province ? this.getDistricts(this.getAreaCode(this.determineType(this.province))) : [] //获取市数据
      this.areas = this.city ? this.getDistricts(this.getAreaCode(this.determineType(this.city), this.area)) : [] //获取区域
    } else {
  ...
  watch: {
    ...
    currentCity(value) {
      // console.log('currentProvince',this.currentProvince) 中文名字
      this.$emit('city', this.setData(value, this.currentProvince))//@city 
  ...
  methods: {
    setData(value, check = '') {
      return {
        code: this.getAreaCode(value, check),//获取code
        value: value,//获取名称
      }
    },
    emit(name) {
    ...
    getAreaCode(name, check = '', ) {// 返回市code
     //
      for(let x in DISTRICTS) {
        for(let y in DISTRICTS[x]) {
          if(name === DISTRICTS[x][y]) {//市名称
            if (check.length > 0) { //当获取市的时候，第一遍是获取省的code,
    ...
    getCodeValue(code) { // 获取打印出来的 value
      for(let x in DISTRICTS) {
        for(let y in DISTRICTS[x]) {
          if(code === parseInt(y)) {//市对应的code
            return DISTRICTS[x][y] //返回市名称
          }
        }
      }
    },
